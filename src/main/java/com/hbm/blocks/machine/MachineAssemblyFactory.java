...
public class MachineAssemblyFactory extends BlockDummyable implements ITooltipProvider, ILookOverlay {
    ...
    // Dimensions/offset analogous to other 5x5 multiblocks (e.g., ChemicalFactory)
    ...
    public void printHook(Pre event, World world, int x, int y, int z) {
        ...
        TileEntityMachineAssemblyFactory fac = (TileEntityMachineAssemblyFactory) te;

        DirPos[] cool = fac.getCoolPos();
        DirPos[] io = fac.getIOPos();

        for(DirPos dirPos : cool) {
            if(dirPos.compare(x + dirPos.getDir().offsetX, y, z + dirPos.getDir().offsetZ)) {
                List<String> text = new ArrayList();
                text.add(EnumChatFormatting.GREEN + "-> " + EnumChatFormatting.RESET + fac.water.getTankType().getLocalizedName());
                text.add(EnumChatFormatting.RED + "<- " + EnumChatFormatting.RESET + fac.lps.getTankType().getLocalizedName());
