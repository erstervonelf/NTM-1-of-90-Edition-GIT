...
public class TileEntityWatz extends TileEntityMachineBase implements IFluidStandardTransceiver, IControlReceiver, IGUIProvider, IFluidCopiable {
	...
	public boolean isOn;
	// preflash triggered when mud tank nears full, prevents repeat spam
	public boolean preFlashTriggered = false;
	...
	public void updateEntity() {
		...
		if(!worldObj.isRemote && !updateLock()) {
			...
			if(sharedTanks[2].getFill() > 0) {
				// clear top area (preserve existing behaviour)
				for(int x = -3; x <= 3; x++) {
					for(int y = 3; y < 6; y++) {
						for(int z = -3; z <= 3; z++) {
				...
				// spawn client particles: keep rbmkmush, add radial smoke, shock and foam
				...
				// additional large radial smoke and shock for visual feedback
				ExplosionLarge.spawnParticlesRadial(worldObj, xCoord + 0.5, yCoord + 2, zCoord + 0.5, 60);
				ExplosionLarge.spawnFoam(worldObj, xCoord + 0.5, yCoord + 2, zCoord + 0.5, 40);
				ExplosionLarge.spawnShock(worldObj, xCoord + 0.5, yCoord + 2, zCoord + 0.5, 20, 2.0);

				// spawn visible debris/shrapnel
				ExplosionLarge.spawnRubble(worldObj, xCoord + 0.5, yCoord + 1.5, zCoord + 0.5, 30);
				ExplosionLarge.spawnShrapnels(worldObj, xCoord + 0.5, yCoord + 1.5, zCoord + 0.5, 20);

				// apply a stronger explosion to increase damage radius
				// strength 8 gives a much larger area and entity damage
				worldObj.createExplosion(null, xCoord + 0.5, yCoord + 2, zCoord + 0.5, 8.0F, true);

				return;
			}

			/* preflash when nearly full: white flash + shake + collapse above reactor */
			int mudMax = sharedTanks[2].getMaxFill();
			int mudFill = sharedTanks[2].getFill();
			if(!preFlashTriggered && mudFill > mudMax * 0.9) {
				preFlashTriggered = true;
				// notify nearby clients (50 blocks) to flash and shake
				NBTTagCompound fx = new NBTTagCompound();
				fx.setString("type", "watz_preflash");
				fx.setInteger("radius", 50);
				PacketDispatcher.wrapper.sendToAllAround(new AuxParticlePacketNT(fx, xCoord + 0.5, yCoord + 2, zCoord + 0.5), new TargetPoint(worldObj.provider.dimensionId, xCoord, yCoord, zCoord, 50));
				MainRegistry.proxy.effectNT(fx);

				// structural collapse above reactor within 50 blocks radius
				int R = 50;
				for(int dx = -R; dx <= R; dx++) {
					for(int dz = -R; dz <= R; dz++) {
						if(dx * dx + dz * dz <= R * R) {
							for(int y = yCoord + 1; y <= Math.min(worldObj.getHeight(), yCoord + R); y++) {
								int bx = xCoord + dx;
								int bz = zCoord + dz;
								if(!worldObj.isAirBlock(bx, y, bz)) {
									worldObj.setBlockToAir(bx, y, bz);
								}
							}
						}
					}
			...
			// allow retrigger later if mud level drops significantly
			if(preFlashTriggered && mudFill < mudMax * 0.5) {
				preFlashTriggered = false;
			}
		}
	}
	...
	public void readFromNBT(NBTTagCompound nbt) {
		...
		// load preflash state to avoid re-triggering immediately after reload
		if(nbt.hasKey("preFlashTriggered")) {
			this.preFlashTriggered = nbt.getBoolean("preFlashTriggered");
		}
	}
	...
	public void writeToNBT(NBTTagCompound nbt) {
		...
		// persist preflash state so it isn't retriggered after world reload
		nbt.setBoolean("preFlashTriggered", this.preFlashTriggered);
	}
